...
results_file = f"logs/xgboost_regression/hyperparameter_tuning/hyperparameter_tuning_{scoring_type}_{timestamp}.txt"
...
with open(results_file, 'w') as f:
    ...
    if len(df_next_season) == 0:
        ...
        predictions_file = f"logs/xgboost_regression/hyperparameter_tuning/predictions_{scoring_type}_{timestamp}.csv"
        df_next_season_predictions.to_csv(predictions_file, index=False)
        print(f"All predictions saved to {predictions_file}")

# === SAVE BEST MODEL ===
model_file = f"xgboost_regression/joblib_files/xgb_pipeline_{best_scaler_name}_{scoring_type}_{timestamp}.pkl"
