...
with open(results_file, 'w') as f:
    ...
    f.write("---------------------------\n\n")
    f.write("Rank | Player Name | Position | Actual | Predicted\n")
    f.write("-----|-------------|----------|---------|----------\n")
    
    for i, (_, row) in enumerate(df_predictions.head(20).iterrows(), 1):
        f.write(f"{i:4d} | {row['Player Name']:11s} | {row['Position']:8s} | {row['Actual']:6.1f} | {row['Predicted']:8.1f}\n")
    
    predictions_file = f"logs/mlp_regression/{timestamp}/test_predictions_{timestamp}.csv"
    ...
    if len(df_next_season) == 0:
        ...
        df_2025_pred = df_next_season[["Player Name", "Position"]].copy()
        ...
        predictions_file_2025 = f"logs/mlp_regression/{timestamp}/predictions_2025.csv"
