...
def create_visualizations(y_train, y_pred_train, y_pred, feature_importance, output_dir, scoring_type):
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/dnn_predictions_training_{scoring_str}.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/dnn_predicted_vs_actual_training_{scoring_str}.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/dnn_predictions_test_{scoring_str}.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/dnn_feature_importance_2024_{scoring_str}.png')
    plt.close()
...
def save_model(model, scaler, output_dir, scoring_type):
    ...
    # Save the model and scaler
    joblib.dump(model, f'{output_dir}/dnn_model_2024_{scoring_str}.joblib')
    joblib.dump(scaler, f'{output_dir}/dnn_scaler_2024_{scoring_str}.joblib')
    
    print("Model and scaler saved successfully.")

def main():
    """
    Main function to run the Deep Neural Network model pipeline.
    """
    ...
    scoring_str = "PPR" if scoring_type == 1 else "Standard"
    print(f"\nRunning Deep Neural Network model pipeline for {scoring_str} scoring...")
    
    # Set paths
    data_path = 'data/final_data/nfl_stats_long_format.csv'
    graphs_dir = 'graphs/deep_neural_network'
    model_dir = 'deep_neural_network/joblib_files'
    ...
    print(f"\nDeep Neural Network model pipeline for {scoring_str} scoring completed successfully.")
